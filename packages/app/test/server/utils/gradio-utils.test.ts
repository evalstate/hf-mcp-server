...
describe('isGradioTool', () => {
	describe('should return true for valid Gradio tool names', () => {
		...
		it('should detect tool names generated by createGradioToolName', () => {
			...
			const publicTool1 = createGradioToolName('evalstate/flux1_schnell', 0, false);
			const publicTool2 = createGradioToolName('abidlabs/EasyGhibli', 1, false);
			const privateTool = createGradioToolName('my-company/private-model', 2, true);
...
describe('createGradioToolName', () => {
	describe('should generate correct tool names', () => {
		it('should create public tool names with gr prefix', () => {
			expect(createGradioToolName('evalstate/flux1_schnell', 0, false)).toBe('gr1_evalstate_flux1_schnell');
			expect(createGradioToolName('user/model', 5, false)).toBe('gr6_user_model');
		});

		it('should create private tool names with grp prefix', () => {
			expect(createGradioToolName('company/private-model', 0, true)).toBe('grp1_company_private_model');
			expect(createGradioToolName('user/secret', 2, true)).toBe('grp3_user_secret');
		});

		it('should sanitize space names correctly', () => {
			expect(createGradioToolName('user/model-name.v2', 0, false)).toBe('gr1_user_model_name_v2');
			expect(createGradioToolName('company/my space/test', 1, false)).toBe('gr2_company_my_space_test');
			expect(createGradioToolName('org/multi--dash__test', 0, false)).toBe('gr1_org_multi_dash_test');
		});
	...
	describe('integration with DEFAULT_SPACE_TOOLS examples', () => {
		it('should generate correct names for known default spaces', () => {
			// These should match the examples used in the existing codebase
			expect(createGradioToolName('evalstate/flux1_schnell', 0, false)).toBe('gr1_evalstate_flux1_schnell');
			expect(createGradioToolName('abidlabs/EasyGhibli', 1, false)).toBe('gr2_abidlabs_easyghibli');
		});
	});
});
