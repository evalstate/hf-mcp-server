...
describe('extractArxivIdFromInput', () => {
	...
	it('should reject invalid formats', () => {
		...
		expect(() => extractArxivIdFromInput('abcd.efgh')).toThrow('URL must be from arxiv.org, huggingface.co, or hf.co. Got: abcd.efgh');
		...
		// Invalid domains
		expect(() => extractArxivIdFromInput('https://example.com/papers/2502.16161')).toThrow('URL must be from arxiv.org, huggingface.co, or hf.co. Got: example.com');
		expect(() => extractArxivIdFromInput('github.com/papers/2502.16161')).toThrow('URL must be from arxiv.org, huggingface.co, or hf.co. Got: github.com');
		
		// Invalid paths on valid domains
		expect(() => extractArxivIdFromInput('https://arxiv.org/pdf/2502.16161')).toThrow('arXiv URL must be in format: arxiv.org/abs/YYMM.NNNNN');
		expect(() => extractArxivIdFromInput('https://hf.co/models/2502.16161')).toThrow('Hugging Face URL must be in format: hf.co/papers/YYMM.NNNNN');
		...
		expect(() => extractArxivIdFromInput('hf.co/papers/2502.16161?foo=bar')).toThrow('URL must contain only the paper ID path');
		expect(() => extractArxivIdFromInput('hf.co/papers/2502.16161#section')).toThrow('URL must contain only the paper ID path');
	});

	it('should reject obvious domain names without path', () => {
		expect(() => extractArxivIdFromInput('hf.co')).toThrow('Hugging Face URL must be in format: hf.co/papers/YYMM.NNNNN');
		expect(() => extractArxivIdFromInput('huggingface.com')).toThrow('URL must be from arxiv.org, huggingface.co, or hf.co. Got: huggingface.com');
	...
	it('should handle edge cases for domain validation', () => {
		...
		expect(() => extractArxivIdFromInput('arxiv.com/abs/2502.16161')).toThrow('URL must be from arxiv.org, huggingface.co, or hf.co. Got: arxiv.com');
		expect(() => extractArxivIdFromInput('huggingface.ai/papers/2502.16161')).toThrow('URL must be from arxiv.org, huggingface.co, or hf.co. Got: huggingface.ai');
		expect(() => extractArxivIdFromInput('hf.ai/papers/2502.16161')).toThrow('URL must be from arxiv.org, huggingface.co, or hf.co. Got: hf.ai');
