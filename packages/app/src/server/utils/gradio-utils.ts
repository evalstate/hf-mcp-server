 ...
 */
export function createGradioToolName(spaceName: string, index: number, isPrivate: boolean | undefined): string {
	...
	// Sanitize the space name: replace special characters with underscores, normalize multiple underscores, and lowercase
	let sanitizedName = spaceName 
		? spaceName
