...
describe('DuplicateSpaceTool', () => {
  describe('normalizeSpaceName', () => {
    ...
    it('should throw error when trying to use different username', () => {
      ...
      expect(() => tool.normalizeSpaceName('bad-user/new-space')).toThrow('Invalid space ID: bad-user/new-space. You can only create spaces in your own namespace. Try "myusername/new-space"');
    });
  ...
  describe('createToolConfig', () => {
    it('should include username in tool description', () => {
      ...
      expect(config.description).toContain('evalstate/space-name');
      expect(config.description).toContain('If you provide just "space-name", it will be created as "evalstate/space-name"');
    });

    it('should handle undefined username in description', () => {
      const config = DuplicateSpaceTool.createToolConfig();
      expect(config.description).toContain('Target space will be created as "space-name"');
